      ...
      <><div>
        <HexGrid width={400} height={200} viewBox="-10 -10 20 20">
          <Layout size={{ x: 2, y: 2 }} flat={false} spacing={1.02} origin={{ x: 0, y: 0 }}>
            {moreHexas.map((hex, i) => <a data-tip data-for="global"> <Hexagon fill={(parseInt(color,16)+i*0x2000).toString(16)} key={i} q={hex.q} r={hex.r} s={hex.s} onMouseLeave={(e, h) => this.onMouseLeave(e, h)} onMouseEnter={(e, h) => this.onMouseEnter(e, h)} />
            </a>)}
          </Layout>
        </HexGrid> 
        </div>
